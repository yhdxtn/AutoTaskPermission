...
if not exist "%GRADLE_BIN%" (
  powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "if ($download) { Write-Host ('正在下载 Gradle ' + $env:GRADLE_VERSION + ' ...'); Invoke-WebRequest -UseBasicParsing -Uri $env:DIST_URL -OutFile $env:ZIP_FILE };" ^
    "$hash=(Get-FileHash -Algorithm SHA256 $env:ZIP_FILE).Hash.ToLowerInvariant(); if ($hash -ne $env:GRADLE_SHA256) { Remove-Item -Force $env:ZIP_FILE; throw 'Gradle 下载文件校验失败' };" ^
